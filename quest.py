...
def draw_game_over():
    ...
    if player_won:
        screen.draw.text("Hai vinto!", midtop=screen_middle, \
                         fontsize=GRID_SIZE, color="green", owidth=1)
    else:
        screen.draw.text("Hai perso!", midtop=screen_middle, \
                         fontsize=GRID_SIZE, color="red", owidth=1)
    screen.draw.text("Premi SPAZIO per rigiocare", midtop=(WIDTH / 2, \
